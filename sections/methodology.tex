...
The baseline IPO is therefore different from the proposed IPO at the process level rather than at the input level. Both systems receive passive RSSI and CSI observations from the same ESP32 anchors, but the baseline treats the fused feature set as one direct input to a final position estimator. The proposed system separates the functional role of each signal type by using RSSI to constrain the candidate area before CSI is applied for final refinement. This distinction is necessary because the related studies establish the usefulness of RSSI, CSI, and feature fusion, but they do not enumerate an implementation sequence identical to the present experimental setup.
...
Algorithm 1 is not presented as a verbatim reproduction of the algorithms used by Yang et al.\ (2013) or Li et al.\ (2024). Rather, it is a reconstructed baseline that operationalizes the signal-fusion logic supported by those studies within the hardware, gateway, and capture-window constraints of this research. Its validity is limited to representing a literature-consistent direct RSSI+CSI comparison model, not to claiming procedural identity with the cited works.
...
\begin{enumerate}
    ...
    \item It must classify observed devices as wanted or unwanted based on the authorized device list used in the trial.
...
Device distinction is performed before localization output is finalized. The gateway maintains an authorized-device list containing the identifiers and test roles of wanted devices. During each capture window, observed packets are grouped by available device identifier and timing behavior. If an observed source matches the authorized-device list, it is tagged as wanted; if it is not listed or is assigned as the hidden target for the trial, it is tagged as unwanted. When wanted and unwanted devices are in close proximity, the system distinguishes them by maintaining separate packet groups per identifier and by comparing their RSSI and CSI observations across anchors before assigning the localization result to the corresponding device group.

The output of each valid capture window is therefore not limited to a coordinate estimate. Each output record contains the device category, the estimated coordinate, the localization mode used, packet sufficiency indicators, latency, and evaluation metrics needed for comparison. The final study output consists of comparative performance tables and logged localization results for the baseline and proposed systems under identical test conditions.
...
These non-functional requirements will be measured empirically during the same trials used for localization comparison. Low cost will be documented through the bill of materials and the number of commodity nodes deployed. Reproducibility will be measured through repeated trials at the same target coordinates and summarized using variance, median error, and inter-trial estimate stability. Robustness will be measured by comparing successful estimate rate, packet sufficiency rate, and error escalation under normal and disturbed conditions. Efficiency will be measured using end-to-end latency and processing time per capture window recorded at the gateway. Maintainability will be assessed through the separation of capture, preprocessing, localization, and logging modules and by documenting whether each module can be tested or modified without changing the entire pipeline.
...
\begin{enumerate}
    ...
    \item trials with wanted and unwanted devices placed at separate and close-proximity positions.
\end{enumerate}
...
Device detection and identification will be evaluated as part of the same controlled testing process. The test cases will include: one wanted device only, one unwanted hidden device only, simultaneous wanted and unwanted devices, and close-proximity placement where both devices transmit within the same observation period. For each case, the gateway will compare observed device identifiers against the authorized-device list, separate packet groups by identifier and capture window, and assign each group a wanted or unwanted classification label before localization results are logged. Classification parameters will include authorization status, cooperation status, visibility status, and stationarity during the trial. Thus, a device may be classified as wanted-cooperative, wanted-non-cooperative, unwanted-visible, or unwanted-hidden depending on the test role assigned before the run.
...
\begin{enumerate}
    ...
    \item \textbf{Step 2A: Device Detection and Classification Trial} \\
    \textit{Purpose:} To verify that the system can distinguish wanted and unwanted devices before localization performance is interpreted. \\
    \textit{Procedure:} The authorized-device list will be loaded into the gateway before each run. Wanted devices, unwanted devices, and close-proximity wanted--unwanted pairs will then be observed across repeated capture windows. The gateway will classify each observed device group according to authorization status, cooperation status, visibility status, and stationarity. \\
    \textit{Expected Outcomes:} Wanted devices should be tagged as authorized sources, while hidden or unregistered sources should be tagged as unwanted targets for localization. Close-proximity devices should remain separated by identifier and capture-window grouping before their location estimates are computed. \\
    \textit{Metrics and Validation:} Classification accuracy, false wanted rate, false unwanted rate, missed detection rate, and correct device-to-location assignment rate will be recorded. Validation will require that classification labels and localization outputs correspond to the correct device group.
